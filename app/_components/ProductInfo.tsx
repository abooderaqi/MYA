...
const ProductInfo = ({ productInfo }: { productInfo: ProductType }) => {
  ...
  const [mainImage, setMainImage] = useState(productInfo.media[0])
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2">
          ...
          <div className="flex justify-center items-center">
            <div className="pro-detail w-full max-lg:max-w-[608px] lg:pl-8 xl:pl-16 max-lg:mx-auto max-lg:mt-8">
              ...
              <ul className="flex gap-3 mb-6 max-w-sm ">
                {productInfo.colors.map((color: string, index: number) => (
                  <li key={index}>
                    <SfChip
                      className="mr-2"
                      slotPrefix={
                        <SfThumbnail style={{ backgroundColor: color }} />
                      }
                    >
                      {color}
                    </SfChip>
              ...
              <p className="font-medium text-lg text-gray-900 mb-2">Sizes</p>
              <div className="grid grid-cols-4 min-[400px]:grid-cols-4 gap-3 mb-3 min-[400px]:mb-8">
                {productInfo.sizes.map((size: string, index: number) => (
                  <button
                    key={index}
                    onClick={() => setSelectedSize(size)}
                    className={`border border-blue-500 whitespace-nowrap text-gray-900 text-sm leading-6 py-2.5 rounded-full px-5 text-center w-full font-semibold shadow-sm shadow-transparent transition-all duration-300 ${
              ...
              <div className="flex items-center mt-6 min-[400px]:flex-row gap-3 mb-3 min-[400px]:mb-8">
